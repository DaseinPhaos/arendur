...
impl<T: Filter> Film<T> {
    ...
    pub fn new(resolution: Point2<usize>, crop_window: BBox2f, filter: T) -> Film<T> {
    ...
    /// spawn tiles
    pub fn spanw_tiles<S>(&self, nx: usize, ny: usize) -> Vec<FilmTile<T, S>>
    ...
    pub fn collect_into<'a, S, I>(&self, tiles: I) -> Image
        where S: Spectrum<Scalar=Float>,
              TilePixel<S>: Clone,
              I: IntoIterator<Item=FilmTile<'a, T, S>>,
              T: 'a,
    {
...
impl<'a, T:'a, S> FilmTile<'a, T, S>
    where T: Filter,
          S: Spectrum + ops::AddAssign,
...
impl Image {
    ...
    pub fn into_inner(self) -> BoundedSink2D<RGBSpectrumf> {
        self.inner
    }
